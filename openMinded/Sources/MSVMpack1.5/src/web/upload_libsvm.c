...
//		printf("%s<br>\n",tmp);
	...
	// Call converter
	char cmd[2000];
	sprintf(cmd, "./convert %s %s%s `cat %s | wc -l` 4",fullfilename,path_to_data,"convert.tmp",fullfilename);

	system(cmd); 
	
	if((fp = fopen("convert.tmp","r") ) == NULL) {
		printf("Error while converting to MSVMpack format. Please check the original file format.<br>\n");
	}
	else {
		fclose(fp);
		rename("convert.tmp",fullfilename);

		printf("<p>File converted from LibSVM to MSVMpack format.</p>\n" );
	}
