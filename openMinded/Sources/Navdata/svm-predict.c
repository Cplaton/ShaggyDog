  ...
  for (l = 0; l < 10; l++)	// pour parcourir le tableau de structure
      ...
      for (i = 0; i < 10; i++)	// pour parcourir la structure
	{
	  ...
	  switch (i)
	    {
	    case 0:
	      x[i].value = buffer[l].pitch;
	      break;
	    case 1:
	      x[i].value = buffer[l].roll;
	      break;
	    case 2:
	      x[i].value = buffer[l].vyaw;
	      break;
	    case 3:
	      x[i].value = buffer[l].vx;
	      break;
	    case 4:
	      x[i].value = buffer[l].vy;
	      break;
	    case 5:
	      x[i].value = buffer[l].vz;
	      break;
	    case 6:
	      x[i].value = buffer[l].ax;
	      break;
	    case 7:
	      x[i].value = buffer[l].ay;
	      break;
	    case 8:
	      x[i].value = buffer[l].az;
	      break;
	    case 9:
	      x[i].index = -1;
	      break;
...
//		printf("apres call svm predict(modell,x)\n");
        ...
        //free(x);
>>>>>>> a65dabe3ac556785cc2b62aaa8492097f5a0604d
	}
      //printf("apres tableau\n");
      predict_label = svm_predict (modell, x);
      //printf("apres call svm predict(modell,x)\n");
...
predict_results
recognition_process (specimen * buffer, char *training_model)
{
  predict_results res;
